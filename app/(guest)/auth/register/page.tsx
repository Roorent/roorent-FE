...
function Register() {
	...
	<div className="w-full min-h-screen flex justify-center relative">
		<Tabs defaultActiveKey="owner" type="card" items={itemsTab} onChange={onChangeTab} activeKey={tabActive} style={{zIndex: 9999999, position: 'absolute', top: '50px', width: '434px', backgroundColor: 'white'}} 
		className="rounded-[20px]"/>
		{tabActive == "owner" ? <div className="absolute w-full h-full"><RegisOwner/></div> : <div className="absolute w-full h-full"><RegisRenter/></div>}
	</div>
	);
}
